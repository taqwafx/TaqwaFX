...
const AffiliateIBDetails = () => {
  const navigate = useNavigate();
  const { affiliateIBId } = useParams();
              ...
              <tbody>
               {data?.data?.upcommingCommisions?.map((repayment, index) =>(
                 <tr   key={index}
                  onClick={() =>
                    navigate(
                      `/admin/investors/${repayment?.investorId}/investment/${repayment?.investmentId}`
                    )
                  } className="bg-white border-b border-gray-200 hover:bg-gray-50 cursor-pointer text-nowrap whitespace-nowrap">
                  <td className="px-6 py-4 font-medium">{repayment?.investorId}</td>
                  <td className="px-6 py-4 font-medium">{repayment?.investorName}</td>
                  <td className="px-6 py-4 font-medium">{repayment?.investmentId}</td>
                  <td className="px-6 py-4 font-medium">{repayment?.repaymentMonthNumber}</td>
                  <td className="px-6 py-4 font-medium">{formatDateToIST(repayment?.repaymentDate)}</td>
                  <td className="px-6 py-4 font-medium">{formatRupee(repayment?.affiliateCommission)}</td>
                  <td className="px-6 py-4 font-medium">{repayment?.planType}</td>
                  <td className="px-6 py-4 font-medium">{formatRupee(repayment?.capital)}</td>
                </tr>
               ))}
              </tbody>
            </table>
            ...
            <tbody>
              {data?.data?.referralUsers.map((referral, key) => (
                <tr
                  ...
                  onClick={() =>
                    navigate(`/admin/investors/${referral?.investorId}`)
                  }
                  className="bg-white border-b border-gray-200 hover:bg-gray-50 cursor-pointer text-nowrap whitespace-nowrap"
                >
                  ...
                  {/* <td className="px-6 py-4 font-medium">
                    {formatDateToIST(referral?.joinDate)}
                  </td> */}
                  <td className="px-6 py-4 font-medium">
...
export default AffiliateIBDetails;
